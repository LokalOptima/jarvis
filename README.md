...
```
src/
  main.cpp              CLI entry — register keywords, run detection
...
wip/                    Pipeline executor (client/server, ops) — being restructured
  main.cpp              Old CLI with hardcoded pipelines (example of library usage)
  ops.cpp/h             Msg, Step, Pipeline, all built-in ops (transcribe, tts, etc.)
  server.cpp/h          TCP server wrapping Jarvis
  client.cpp/h          TCP client — audio streaming, local step dispatch
  net.h                 TCP framing protocol, dual-stack sockets
  weather.cpp/hpp       Weather fetch + TTS
  jarvis-client.cpp     Lightweight mic streamer binary
...
data/
  ...
  clips/                Per-keyword enrollment clips (gitignored)
```

## API

Jarvis is a C++ library with a simple callback interface:

```cpp
#include "jarvis.h"

Jarvis j("models/ggml-tiny.bin", "models/silero_vad.bin");
j.set_ding("data/beep.wav");

j.add_keyword({"hey_jarvis", "models/templates/hey_jarvis.bin", 0.35f});
...
./build/jarvis [options]
  --model PATH       Whisper model      (default: models/ggml-tiny-Q8.bin)
