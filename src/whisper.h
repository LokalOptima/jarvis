...
extern "C" {
    ...
    // Get encoder output after whisper_encode().
    // Copies encoder hidden states to output buffer.
    // Returns number of floats written, or -1 on error.
    WHISPER_API int whisper_encoder_output(struct whisper_context * ctx, float * output, int max_floats);

    // Set encoder context length for variable-length encoding.
    // Call before whisper_encode() to process only n_audio_ctx frames instead of the full 1500.
